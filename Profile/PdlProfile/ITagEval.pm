...
##     #do_ambig=>$bool,       ##-- whether to track ambiguity data: OBSOLETE
##
##     ##-- runtime data (OLD)
##     #ntoks => $ntokens,
...
##     jpdist  => $sparse,      ##-- MUDL::PdlDist::Sparse2d: <tag1,tag2> => f(tag1 && tag2)
##     #jpdense => $dense,      ##   + ... the dense version
##     tag1txt => $sparse,      ##-- MUDL::PdlDist::Sparse2d: <tag1,txt>  => f(txt  && tag1)
##     tag2txt => $sparse,      ##-- MUDL::PdlDist::Sparse2d: <tag2,txt>  => f(txt  && tag2)
##
##     ##-- after finishPdlProfile() .... ???
...
sub finishPdlProfile {
  ...
  if (defined($eval->{targets})) {
    ...
    $Ntxtk   = $tgs_to_enum2->nelem;             ##-- number of known and found text types
  ...
  ## Common variables
  ##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  my ($tag1id,$tag1,$tag2,$tag2id,$tagi, $pr,$rc,$F);
  ...
  ##-- initialize: tag1i
  {
    my @tag1freq   = $tag1dp->list;
    my @tag12bestf = $tag12bestf->list;
    my @tag12best  = $tag12best->list;
    my @nbest1i    = $nbest1i->list;
    my @nunk1      = $nunk1->list;
    foreach $tag1id (0..$#$tag1syms) {
      $tag1i->{$tag1syms->[$tag1id]} = $tag1ia->[$tag1id] =
	{
	 freq     => $tag1freq[$tag1id],
	 fbest    => $tag12bestf[$tag1id],
	 tbest    => $tag2syms->[$tag12best[$tag1id]],
	 nbesti   => $nbest1i[$tag1id],
	 nunknown => $nunk1[$tag1id],
	};
    }
  }
  ##-- initialize: tag2i
  {
    my @tag2freq   = $tag2dp->list;
    my @tag21bestf = $tag21bestf->list;
    my @tag21best  = $tag21best->list;
    my @nbest2i    = $nbest2i->list;
    my @nunk2      = $nunk2->list;
    foreach $tag2id (0..$#$tag2syms) {
      $tag2i->{$tag2syms->[$tag2id]} = $tag2ia->[$tag2id] =
	{
	 freq     => $tag2freq[$tag2id],
	 fbest    => $tag21bestf[$tag2id],
	 nbesti   => $nbest2i[$tag2id],
	 tbest    => $tag2syms->[$tag21best[$tag2id]],
	 nunknown => $nunk2[$tag2id],
	};
    }
  }
  ...
  my $meta_ncor12_by2 = ($jpdl * $isbest12 * $tag1kmask)->sumover;
  ...
  {
    my @pr=$meta_pr_by1->list;
    my @rc=$meta_rc_by1->list;
    my @F =$meta_F_by1->list;
    foreach (0..$#$tag1ia) {
      @{$tag1ia->[$_]}{qw(meta_precision meta_recall meta_F)} = ($pr[$_],$rc[$_],$F[$_]);
    }
    @pr=$meta_pr_by2->list;
    @rc=$meta_rc_by2->list;
    @F =$meta_F_by2->list;
    foreach (0..$#$tag2ia) {
      @{$tag2ia->[$_]}{qw(meta_precision meta_recall meta_F)} = ($pr[$_],$rc[$_],$F[$_]);
    }
  }

  ##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ## meta-tagging precision,recall: get totals
  $pr = $eval->{meta_precision} = fracpdl($meta_ncor12_by1->sumover, $ftotal)->sclr;
  $rc = $eval->{meta_recall}    = fracpdl($meta_ncor21_by1->sumover, $ftotal)->sclr;
  ...
  ##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ## CONTINUE HERE: tag-wise precision, recall (Sch"utze-style)
