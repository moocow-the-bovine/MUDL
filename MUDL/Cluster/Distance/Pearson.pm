...
sub compare {
  ...
  my $mu1    = $args{data1}->average;
  my $mu2    = $args{data2}->average;
  my $sigma1 = ($args{data1} - $mu1->slice("*1"))->pow(2)->average->sqrt;
  my $sigma2 = ($args{data2} - $mu2->slice("*1"))->pow(2)->average->sqrt;

  my $mu1r = $mu1->index($args{rows1})->slice("*1");
  my $mu2r = $mu2->index($args{rows2})->slice("*1");
  my $sigma1r = $sigma1->index($args{rows1})->slice("*1");
  my $sigma2r = $sigma2->index($args{rows2})->slice("*1");

  $cmpvec .=  1 - (($dr1-$mu1r)/$sigma1r * ($dr2-$mu2r)/$sigma2r)->sumover / $d;
